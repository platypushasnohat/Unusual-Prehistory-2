...
public class TundraFossilPiece extends TemplateStructurePiece {

    public static final Map<ResourceLocation, Integer> HEIGHT_TO_TEMPLATES = Util.make(Maps.newHashMap(), map -> {
        map.put(UnusualPrehistory2.modPrefix("fossil/tundra_1"), 7);
    });

    public TundraFossilPiece(StructureTemplateManager manager, ResourceLocation id, BlockPos pos, Rotation rotation) {
        super(UP2StructurePieces.TUNDRA_FOSSIL.get(), HEIGHT_TO_TEMPLATES.get(id), manager, id, id.toString(), createPlacementData(rotation), pos);
    }

    public TundraFossilPiece(StructureTemplateManager manager, CompoundTag compoundTag) {
        super(UP2StructurePieces.TUNDRA_FOSSIL.get(), compoundTag, manager, id -> createPlacementData(Rotation.valueOf(compoundTag.getString("Rotation"))));
    }

    public TundraFossilPiece(StructurePieceSerializationContext context, CompoundTag compoundTag) {
    ...
    public static void addPieces(StructureTemplateManager manager, BlockPos pos, StructurePieceAccessor holder, RandomSource random) {
        ...
        holder.addPiece(new TundraFossilPiece(manager, randomTemplate, pos, rotation));
    }
    ...
    private static CappedProcessor matrixProcessor(ResourceLocation lootTable, String rarity, int cap) {
        return new CappedProcessor(new MatrixProcessor(Blocks.SNOW_BLOCK.defaultBlockState(), UP2Blocks.SNOW_MATRIX.get().defaultBlockState(), lootTable, rarity), ConstantInt.of(cap));
    }
}