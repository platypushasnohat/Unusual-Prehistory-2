...
import com.barlinc.unusual_prehistory.client.models.entity.mob.update_4.BarinasuchusModel;
...
public class BarinasuchusRiderLayer extends RiderLayer<Barinasuchus, BarinasuchusModel> {

    public BarinasuchusRiderLayer(RenderLayerParent<Barinasuchus, BarinasuchusModel> parent) {
        super(parent);
    }

    @Override
    public void render(@NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int packedLight, Barinasuchus entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        float bodyYaw = entity.yBodyRotO + (entity.yBodyRot - entity.yBodyRotO) * partialTicks;
        if (entity.isVehicle()) {
            Vec3 offset = new Vec3(0, 0.12F, 0);
