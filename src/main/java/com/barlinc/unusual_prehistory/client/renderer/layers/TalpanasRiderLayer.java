...
public class TalpanasRiderLayer extends RiderLayer<Talpanas, TalpanasModel> {

    public TalpanasRiderLayer(RenderLayerParent<Talpanas, TalpanasModel> parent) {
        super(parent);
    }

    @Override
    public void render(@NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int packedLight, Talpanas entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        float bodyYaw = entity.yBodyRotO + (entity.yBodyRot - entity.yBodyRotO) * partialTicks;
        if (entity.isVehicle()) {
            Vec3 offset = new Vec3(0, 0.1F, 0.2F);
