...
public class MegalaniaRiderLayer extends RiderLayer<Megalania, MegalaniaModel> {

    public MegalaniaRiderLayer(RenderLayerParent<Megalania, MegalaniaModel> parent) {
        super(parent);
    }

    @Override
    public void render(@NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int packedLight, Megalania entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        float bodyYaw = entity.yBodyRotO + (entity.yBodyRot - entity.yBodyRotO) * partialTicks;
        if (entity.isVehicle()) {
            Vec3 offset = new Vec3(0, 0.35F, 0);
            ...
            for (Entity passenger : entity.getPassengers()) {
                ...
                this.getParentModel().body_upper.translateAndRotate(poseStack);
