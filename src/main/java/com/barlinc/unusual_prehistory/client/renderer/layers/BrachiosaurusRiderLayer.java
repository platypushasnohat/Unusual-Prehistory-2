...
public class BrachiosaurusRiderLayer extends RiderLayer<Brachiosaurus, BrachiosaurusModel> {

    public BrachiosaurusRiderLayer(RenderLayerParent<Brachiosaurus, BrachiosaurusModel> parent) {
        super(parent);
    }

    @Override
    public void render(@NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int packedLight, Brachiosaurus entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        float bodyYaw = entity.yBodyRotO + (entity.yBodyRot - entity.yBodyRotO) * partialTicks;
        if (entity.isVehicle()) {
            Vec3 offset = new Vec3(0, 0.0F, 0.5F);
