 ...
 *
 * Source: SmartBrainLib - https://github.com/Tslat/SmartBrainLib/tree/1.21
 * Modifications by: Unusual Prehistory - 5/8/2026
 */
...
public interface ExtendedNavigator {
    ...
    default boolean isCloseToNextNode(float distance) {
        final Mob mob = this.getMob();
        final Path path = this.getPath();
        final Vec3 nextNodePos = this.getEntityPosAtNode(path.getNextNodeIndex());
    ...
    default boolean isCollisionFreeTraversal(Vec3 traversalVector, Vec3 minBoundsPos, Vec3 leadingEdgePos) {
        ...
        for (Direction.Axis axis : Direction.Axis.values()) {
            ...
            final float axisLength = this.lengthForAxis(traversalVector, axis);
            final boolean isPositive = axisLength >= 0;
            final float maxPos = this.lengthForAxis(isPositive ? leadingEdgePos : minBoundsPos, axis);
            ...
            do {
                ...
                for (int x = xBound; x != xStepBound; x += xStep) {
                    for (int z = zBound; z != zStepBound; z += zStep) {
                        ...
                        if (!this.canPathOnto(nodeEvaluator.getPathType(new PathfindingContext(level, mob), x, yBound - 1, z))) {
                            return false;
                        }
                        ...
                        if (this.canPathInto(insidePathType)) {
                            return false;
                        }
                    }
                }
