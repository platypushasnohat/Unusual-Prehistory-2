...
public class TherizinosaurusAttackGoal extends AttackGoal {
    ...
    public void tick() {
        ...
        if (target != null) {
            ...
            if (this.isInChargingPose() || attackState == 1 || attackState == 3) therizinosaurus.getNavigation().stop();
            ...
            if (attackState == 1) {
                ...
                } else if (distance < 70 && therizinosaurus.chargeCooldown == 0 && !this.isInChargingPose() && !therizinosaurus.isInWater() && therizinosaurus.isWithinYRange(target)) {
    ...
    protected void tickSlash() {
        ...
        if (timer > 21) {
            this.timer = 0;
            this.therizinosaurus.slashCooldown = 3 + therizinosaurus.getRandom().nextInt(3);
    ...
    protected void tickSlashRush() {
        ...
        if (timer == 6 || timer == 12 || timer == 18 || timer == 24 || timer == 30) {
            this.therizinosaurus.swing(InteractionHand.MAIN_HAND);
            this.therizinosaurus.doHurtTarget(target);
        }
        if (timer > 40) {
    ...
    protected void tickCharge() {
        ...
        if (timer == 1) therizinosaurus.setPose(UP2Poses.START_CHARGING.get());
        if (timer == 18) therizinosaurus.playSound(UP2SoundEvents.CARNOTAURUS_CHARGE.get(), 1.0F, therizinosaurus.getVoicePitch());
