...
public class SmartBodyHelper extends BodyRotationControl {
    ...
    public void clientTick() {
        ...
        if (entity.getTarget() != null) {
            ...
            this.entity.yBodyRot = approachAngle(entity.yBodyRot, targetAngle, bodyLagMoving, bodyMax);
            this.entity.yHeadRot = approachAngle(entity.yHeadRot, targetAngle, headLag, headMax);
            this.clampHeadBodyDifference();
        } else if (distSqr > MOVE_THRESHOLD) {
            float moveAngle = (float)(Math.toDegrees(Math.atan2(dz, dx)) - 90.0F);
            this.entity.yBodyRot = approachAngle(entity.yBodyRot, moveAngle, bodyLagMoving, bodyMax);
            this.entity.yHeadRot = approachAngle(entity.yHeadRot, entity.yBodyRot, headLag, headMax);
            this.clampHeadBodyDifference();
        } else {
            this.entity.yBodyRot = approachAngle(entity.yBodyRot, entity.yHeadRot, bodyLagStill, bodyMax);
            this.clampHeadBodyDifference();
        }
    }
