...
import com.unusualmodding.unusual_prehistory.mixins.StructureTemplateAccessor;
...
public class NbtFeature<T extends NbtFeatureConfig> extends Feature<T> {
    ...
    public boolean place(FeaturePlaceContext<T> context) {
        ...
        if (template.isEmpty()) {
            LOGGER.error("Identifier to the specified nbt file was not found! : {}", nbtRL);
            return false;
        }
        ...
        template.get().placeInWorld(context.level(), mutable, mutable, placementsettings, context.random(), Block.UPDATE_ALL);
        ...
        List<StructureTemplate.StructureBlockInfo> list = placementsettings.getRandomPalette(((StructureTemplateAccessor) template.get()).Unusualprehistory2$getBlocks(), mutable).blocks();
