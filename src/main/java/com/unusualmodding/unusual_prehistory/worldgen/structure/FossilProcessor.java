...
public class FossilProcessor extends StructureProcessor {
    ...
    public static final Codec<FossilProcessor> CODEC = Codec.unit(FossilProcessor::new);
    ...
    public StructureTemplate.StructureBlockInfo processBlock(LevelReader level, BlockPos blockPos, BlockPos pos, StructureTemplate.StructureBlockInfo structureBlockInfo, StructureTemplate.StructureBlockInfo info, StructurePlaceSettings settings) {
        ...
        if (blockState.is(UP2BlockTags.FOSSILIZED_BONE_BLOCKS) && level.getBiome(blockPos).is(Biomes.LUSH_CAVES) && random.nextFloat() < 0.1F){
            return new StructureTemplate.StructureBlockInfo(info.pos(), Blocks.MOSS_BLOCK.defaultBlockState(), info.nbt());
        }

        if (blockState.is(UP2BlockTags.FOSSILIZED_BONE_BLOCKS) && level.getBiome(blockPos).is(Biomes.DRIPSTONE_CAVES) && random.nextFloat() < 0.1F){
            return new StructureTemplate.StructureBlockInfo(info.pos(), Blocks.DRIPSTONE_BLOCK.defaultBlockState(), info.nbt());
        }

        if (blockPos.getY() < 0) {
            if (blockState.is(UP2BlockTags.FOSSILIZED_BONE_BLOCKS) && random.nextFloat() < 0.08F) {
    ...
    protected StructureProcessorType<?> getType() {
        return UP2StructureProcessorTypes.FOSSIL.get();
    }
}