...
import com.unusualmodding.unusual_prehistory.client.animations.diplocaulus.DiplocaulusSalamandroidesAnimations;
...
public class DiplocaulusSalamandroidesModel<T extends DiplocaulusEntity> extends UP2Model<T> {
	...
	public void setupAnim(DiplocaulusEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (entity.isInWaterOrBubble()) {
			this.animateWalk(DiplocaulusSalamandroidesAnimations.SWIM, limbSwing, limbSwingAmount, 2.0f, 1.0f);
			this.animateIdle(entity.idleAnimationState, DiplocaulusSalamandroidesAnimations.SWIM_IDLE, ageInTicks, 1.0f, 1 - Math.abs(limbSwingAmount));
		}

		if (!entity.isInWaterOrBubble()) {
			this.animateWalk(DiplocaulusSalamandroidesAnimations.WALK, limbSwing, limbSwingAmount, 3.0f, 3.0f);
		...
		this.animate(entity.quirkAnimationState, DiplocaulusSalamandroidesAnimations.QUIRK, ageInTicks, 1.0f);
