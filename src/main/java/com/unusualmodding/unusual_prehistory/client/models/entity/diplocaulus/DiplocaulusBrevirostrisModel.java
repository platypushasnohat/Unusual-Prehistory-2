...
public class DiplocaulusBrevirostrisModel<T extends DiplocaulusEntity> extends UP2Model<T> {
	...
	public void setupAnim(DiplocaulusEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (entity.isInWaterOrBubble()) {
			this.animateWalk(DiplocaulusBrevirostrisAnimations.SWIM, limbSwing, limbSwingAmount, 2.0f, 1.0f);
			this.animateIdle(entity.idleAnimationState, DiplocaulusBrevirostrisAnimations.SWIM_IDLE, ageInTicks, 1.0f, 1 - Math.abs(limbSwingAmount));
		}

		if (!entity.isInWaterOrBubble()) {
			this.animateWalk(DiplocaulusBrevirostrisAnimations.WALK, limbSwing, limbSwingAmount, 3.0f, 3.0f);
