...
import com.unusualmodding.unusual_prehistory.entity.Scaumenacia;
...
@OnlyIn(Dist.CLIENT)
public class ScaumenaciaRenderer extends MobRenderer<Scaumenacia, ScaumenaciaModel<Scaumenacia>> {
    ...
    @Override
    protected void scale(Scaumenacia entity, PoseStack matrices, float amount) {
        if (entity.isBaby()) matrices.scale(0.6F, 0.6F, 0.6F);
        else super.scale(entity, matrices, amount);
    }

    @Override
    public ResourceLocation getTextureLocation(Scaumenacia entity) {
