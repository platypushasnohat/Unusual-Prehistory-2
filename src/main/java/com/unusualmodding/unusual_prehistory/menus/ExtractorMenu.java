...
public class ExtractorMenu extends AbstractContainerMenu {
    ...
    public ExtractorMenu(int containerId, Inventory inventory, FriendlyByteBuf data) {
        this(containerId, inventory, inventory.player.level().getBlockEntity(data.readBlockPos()), new SimpleContainerData(8));
    }

    public ExtractorMenu(int containerId, Inventory inventory, BlockEntity blockEntity, ContainerData data) {
        super(UP2MenuTypes.ANALYZER_MENU.get(), containerId);
        ...
        this.blockEntity.getCapability(ForgeCapabilities.ITEM_HANDLER).ifPresent(handler -> {
            ...
            this.addSlot(new UP2ResultSlot(handler, 2, 35, 52));
            this.addSlot(new UP2ResultSlot(handler, 3, 53, 52));
            this.addSlot(new UP2ResultSlot(handler, 4, 71, 52));
            this.addSlot(new UP2ResultSlot(handler, 5, 89, 52));
            this.addSlot(new UP2ResultSlot(handler, 6, 107, 52));
            this.addSlot(new UP2ResultSlot(handler, 7, 125, 52));
        });
        addDataSlots(data);
    }
    ...
    private static final int SLOT_COUNT = HOTBAR_SLOT_COUNT + PLAYER_INVENTORY_SLOT_COUNT;
    private static final int FIRST_SLOT_INDEX = 0;

    private static final int EXTRACTOR_FIRST_SLOT_INDEX = FIRST_SLOT_INDEX + SLOT_COUNT;
    private static final int EXTRACTOR_SLOT_COUNT = 8;
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (index < FIRST_SLOT_INDEX + SLOT_COUNT) {
            if (!moveItemStackTo(sourceStack, EXTRACTOR_FIRST_SLOT_INDEX, EXTRACTOR_FIRST_SLOT_INDEX + EXTRACTOR_SLOT_COUNT, false)) {
                return ItemStack.EMPTY;
            }
        } else if (index < EXTRACTOR_FIRST_SLOT_INDEX + EXTRACTOR_SLOT_COUNT) {
