...
public class ItemWeightedPairCodec {
    ...
    public static Codec<ItemWeightedPairCodec> CODEC = RecordCodecBuilder.create(inst -> inst.group(BuiltInRegistries.ITEM.byNameCodec().fieldOf("item").forGetter(i -> i.item), Codec.INT.fieldOf("weight").forGetter(w -> w.weight), Codec.INT.optionalFieldOf("amount", 1).forGetter(a -> a.amount)).apply(inst, ItemWeightedPairCodec::new));
