...
public class ExtractorRecipeJsonManager extends SimpleJsonResourceReloadListener {

    private static final Gson STANDARD_GSON = new Gson();
    public static final Logger LOGGER = LogManager.getLogger();

    protected static Map<Item, List<ItemWeightedPairCodec>> recipeList = new HashMap<>();
    protected static Map<ResourceLocation, ExtractorRecipeCodec> recipeListRl = new HashMap<>();
    private final String folderName;

    public ExtractorRecipeJsonManager() {
        this("unusual_prehistory/recipes/extracting", STANDARD_GSON);
    }

    public ExtractorRecipeJsonManager(String folderName, Gson gson) {
    ...
    public static void setRecipeList(Map<Item, List<ItemWeightedPairCodec>> recipeList) {
        ExtractorRecipeJsonManager.recipeList = recipeList;
    }
    ...
    public static void populateRecipeMap(Level level) {
        ...
        for (ResourceLocation resourceLocation : recipeListRl.keySet()) {
            ExtractorRecipeCodec analyzerRecipeCodec = recipeListRl.get(resourceLocation);
    ...
    protected void apply(Map<ResourceLocation, JsonElement> jsons, ResourceManager pResourceManager, ProfilerFiller pProfiler) {
        recipeList.clear();
        Map<ResourceLocation, ExtractorRecipeCodec> recipeList = new HashMap<>();
        for (Map.Entry<ResourceLocation, JsonElement> entry : jsons.entrySet()) {
            ResourceLocation key = entry.getKey();
            JsonElement element = entry.getValue();
            ExtractorRecipeCodec.CODEC.decode(JsonOps.INSTANCE, element)
                    .get()
                    .ifLeft(result -> {
                        ExtractorRecipeCodec analyzerRecipe = result.getFirst();
